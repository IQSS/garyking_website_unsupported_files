 ...
 /*   AUTHOR: Paolo Berta - Matteo Bossi - Stefano Verzillo                           */
 /*************************************************************************************/
...
/*-------------------------------------------------------------------------------------
 ...
 Requires SAS/STAT.
 Citation: Berta, P. & Bossi, M. & Verzillo, S. (2017). '% CEM: A SAS Macro to perform Coarsened Exact Matching', 
 Journal of Statistical Computation and Simulation, Vol. 87(2): pp 227-238;

 SUPPORT
 -------
 The authors are available for support %CEM Macro users.
 Please send emails to paolo.berta@unimib.it and stefano.verzillo@jrc.ec.europa.eu with any suggestion or correction.

 HISTORY
 -------
 initial coding                                               		01Sep2013 VBeBo
 a completely revised version of L1 nested macro            		15Aug2014 VBeBo
 various changes and addictions                                		01May2015 VBeBo
 checking association between treatment and categorical vars        30May2015 VBeBo
 various changes and addictions 							        30Oct2015 VBeBo
 keep option added 													19Dec2016 VBeBo
 how to use macro %CEM in a Cloud environment						07Feb2017 VBeBo
 ...
 USAGE NOTES
 -----------
 For ease of use, copy the obtained file to your preferred folder and then submit the following code before calling the %CEM macro: 

 options source2;
 %include "c:/yourfolder_path/macro_cem_updated_new_feb17.sas"; 
 ...
 If your data are stored in a Cloud environment (Dropbox, Google Drive, ...) an error massage as "ERROR: A lock is not available for..." could appear. 
 This is the way SAS works when a process from a different application is locking your dataset (e.g. Drobpox is syncronizing your original dataset). 
 You have three ways to avoid this problem:
 1) Switch off the sync activity of your Cloud system during the %CEM execution.
 2) Move your dataset to a library not sotred on your Cloud system.
 3) Define your permanent library adding the "FILELOCKWAIT" option to the libname instruction. E.g. LIBNAME lib "path" filelockwait=100; 
    You can find more information about the "FILELOCKWAIT" option on the SAS support webpage: http://support.sas.com/kb/42/240.html
 ...
 * lib: SAS library containing the original dataset; We reccomend to do not store your original dataset in the work library.

 * dataset: name of the original dataset to be read. 
 ...
 * keep: select the variable names to be included as matching covariates;
 ...
 * method: default is "Sturges", that perform the Sturges rule as binning algorithm of numeric variables in order to create Cem strata.
   The alternative is "Percent", that perform a set of automatic coarsening cutpoints of numeric variables (quartiles, quintiles, deciles, centiles and   percentiles). %CEM 
   selects the best combination according to the L1 matching frontier and adds a new column on the original dataset with the corresponding weights. At the same time %CEM stores 
   in the Work library the results of all the performed solutions in several datasets named Cod_dataset_`k' (where `k' is the combination id). 

 * path_graph: option to assign the path where %CEM stores the matching frontier; Do not include the path within quotation marks.
...
/*****************************************/
proc datasets; contents data= keepdataset out= Name_D (keep = NAME TYPE);run;
/*****************************************/
...
%ABORT;
%end;
proc sort data=keepdataset (keep = &Treat.) out=check_value nodupkey; by &Treat.; run;
data check_value;
set check_value;
rename &Treat. = value;
num = _N_;
...
%end;
proc sort data=keepdataset out=control nodupkey; by &id; run;
data _NULL_;
set keepdataset;
...
/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
proc datasets ;
contents data= keepdataset out=Name_D(keep=NAME TYPE);
run;
...
%DO v = 1 %to &num_cat.;
proc freq data = keepdataset; table &&namecat&v. * &treat. /out=CheckCat; run;
...
proc rank data=keepdataset out=ranked4 (keep = %do i = 1 %to &numtot;
&&var_&i
%end;
%do i = 1 %to &numtot;
v_&i._4
...
%do b = 1 %to &numtot;
	proc means data=keepdataset (keep = &&var_&b) stddev min max noprint nway;
...
proc freq data=keepdataset noprint;
...
;
merge V_CAT_&r. (in=a) keepdataset (in=b);
by &label2.;
if a and b;
run;
%end;
...
proc sort data=keepdataset; by &ID; run;
data keepdataset;
update keepdataset cod1_dataset_&numero.;
by &ID;
run;
proc sort data=&lib..&dataset.; by &id; run;												
proc sort data=keepdataset(keep = &ID matched Wsc1_&numero.) out=tmp; by &ID; run;		
data tmp1;																				
	merge &lib..&dataset. tmp;															
	by &ID;																				
run;																					
data &lib..&dataset.;																	
	set tmp1;																			
...
run;
proc sort data=&lib..&dataset.; by &ID; run;												
proc sort data=keepdataset(keep = &ID matched) out=tmp; by &ID; run;						
data tmp1;																				
	merge &lib..&dataset. tmp;															
	by &ID;																				
run;																					
...
ods listing;
goptions reset=all ; 
FILENAME file "&path_graph.\L1_&dataset..png";
